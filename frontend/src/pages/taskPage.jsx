...
function TaskPage(props) {
    ...
    useEffect(() => {
        ...
        else if (props.task.task.Status!=='Open' && (props.task.task.TaskPosterId != props.user.givenId &&props.task.task.UserAcceptedOffer != props.user.givenId)) {
            navigate(`/`);
        }
        ...
        <div className="h-full w-full max-w-[500px] bg-black text-white flex flex-col py-4 ">
            ...
            <div className={`topNavBar flex w-full justify-between items-center mt-4 px-4`}>
                <div className="back-to-main-page flex pt-[0.2rem]">
                    ...
                    <div className="text-white text-3xl ml-5 font-semibold">{toggle ? "Mark as Complete" : 'Task Brief'}</div>
                </div>
                    ...
                    <form className="flex flex-col w-full h-full mt-3" onSubmit={(e) => { handleFeedback(e) }}>
                        <div className="ques flex flex-col">
                            <div className="flex items-baseline mt-2 mb-3 flex-wrap">
                                ...
                                {error&&feedback.rating===0 && <div className="text-sm text-red-900 ml-2 pb-0.5 ">Please select a rating</div>}
                            </div>
                            <div className="flex gap-2">
                                    {[...Array(feedback.rating)].map((_, i) => (
                                        <span key={i} onClick={() => setFeedback({ ...feedback, rating: i + 1 })}>
                                            <StarFilled/>
                                        </span>
                                    ))}
                                    {[...Array(5 - feedback.rating)].map((_, i) => (
                                        <span key={i} onClick={() => setFeedback({ ...feedback, rating: feedback.rating+i + 1 })}>
                                            <EmptyStar/>
                                        </span>
                                    ))}
                        ...
                        <div className="ques flex flex-col">
                            <div className="flex items-center mt-2 mb-3 flex-wrap">
                                ...
                                <div className=" text-sm text-[#777777] self-center ml-2">( Optional )</div>
                            </div>
                        ...
                        <div className="endbar mb-4 mt-auto flex-col items-center">
                            <div className="flex justify-center mt-4 sm:mt-4 w-full">
                                <button type="submit" className={` ${feedback.rating == 0 ? "opacity-[0.2] cursor-not-allowed" : "cursor-pointer"} bg-white text-black w-[95%] h-[2.75rem] font-bold py-1 px-4 rounded-md`} >
                                    Mark as Completed
                                </button>
                            </div>
                        </div>
            ...
            : <div className="mainbrief flex flex-col w-full h-full px-4">
                <div className="flex flex-col w-full mt-8 ">
                    <div className="text-[#a1a1a1] text-[0.8rem]">{props.task.task.categoryName.toUpperCase()}</div>
                    <div className="flex  justify-between">
                        <div className="text-white text-lg font-semibold mt-1.5 max-w-10/12">{props.task.task.title}</div>
                        <div>
                            {props.task.task.Status == "Open" &&
                                <span className={`px-3 py-0.5 flex justify-center items-center rounded-lg border-3 border-[#33c300] text-[#33c300] text-sm font-medium`}>
                                    OPEN
                                </span>
                            }
                            {props.task.task.Status == "In-Progress" &&
                                <span className={`px-3 py-0.5 flex justify-center items-center rounded-lg border-3 border-[#f8f54a] text-[#f8f54a] text-sm font-medium`}>
                                    IN-PROGRESS
                                </span>
                            }
                            {props.task.task.Status == "Completed" &&
                                <span className={`px-3 py-0.5 flex justify-center items-center rounded-lg border-3 border-[#36daff] text-[#36daff] text-sm font-medium`}>
                                    Completed
                                </span>
                            }
                ...
                <div className="flex flex-col w-full mt-5">
                    ...
                    <div className="flex  justify-between mt-3">
                        ...
                        <div className="text-2xl font-semibold whitespace-nowrap">
                            <span className="text-2xl mr-1">₹</span>{props.task.task.budget}
                        </div>
                        ...
                        <div className="flex max-w-1/2">
                            <div className="flex flex-col  justify-center">
                                <div className=" font-semibold text-White text-lg">
                                    {(new Date(props.task.task.endDate)).toLocaleDateString("en-GB", { day: "numeric", month: "long" })}
                                </div>
                                <div className="text-[#a1a1a1] text-[0.75rem]">
                                    {props.task.task.timeSlot} {props.task.task.timeSlot === "Morning" ? "(Before 11 AM)" : props.task.task.timeSlot === "Afternoon" ? "(11 AM - 4 PM)" : props.task.task.timeSlot === "Evening" ? "(After 4 PM)" : ""}
                                </div>
                            </div>
                        </div>
                        <div className="text-xl max-w-1/2 text-right">
                            {props.task.task.modeOfTask == "Remotely" ? "Remote" : `${props.task.task.taskCity}, ${props.task.task.taskLocation}`}
                        </div>
                ...
                {(props.task.task.TaskPosterId == props.user.givenId || props.task.task.UserAcceptedOffer==props.user.givenId) && props.task.task.OfferIdAccepted &&
                    <>
                        ...
                        <div className="flex flex-col w-full mt-5">
                            <div className="flex justify-between">
                                <div className="text-[#a1a1a1] text-[0.8rem]">ACCEPTED BY</div>
                                <div className="text-[#a1a1a1] text-[0.8rem]">FINAL PRICE</div>
                            </div>
                            <div className="flex  justify-between mt-3">
                                <div className="flex">
                                ...
                                <div className="text-2xl font-semibold whitespace-nowrap">
                                    <span className="text-2xl mr-1">₹</span>{props.task.offer.price}
                                </div>
                        ...
                        <div className="flex flex-col w-full mt-5">
                            <div className="text-[#a1a1a1] text-[0.8rem]">OFFER MESSAGE</div>

                            <div className="text-sm mt-3 text-[#777777]">{props.task.offer.description}</div>
                        </div>
                ...
                }
                {
                    ...
                    props.task.task.TaskPosterId == props.user.givenId ?
                        ...
                        :
                        props.task.task.Status=="Open"? <div className="mt-auto mb-6 flex w-full justify-evenly">
                            <div className="flex h-10 w-5/12 bg-[#222225] rounded-lg border-2 font-semibold cursor-pointer border-[#777777] justify-center items-center" onClick={(e) => { e.preventDefault(); navigate(`/messages/${props.task.task.TaskPosterId}`) }}>
                                Send Message
                            </div>
                            <div className="flex h-10 w-5/12 bg-white rounded-lg text-black font-bold justify-center cursor-pointer items-center" onClick={() => { navigate(`/task/offer/${props.task.task.givenId}`) }}>
                                Make An Offer
                            </div>
                        </div>:""}
            </div>}
